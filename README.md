...
按步骤学习与实施见[State 微调实战与算力预算教程](docs/lessons/10-State微调实战与算力预算.md)：包含分批数据下载单、预训练权重选择、`init_from` 微调命令、基因/靶点迁移、原始计数生成、H1 评分、打包，以及从单张 24 GB GPU 起步的资源测量方法。原生命令与需要自行实现的适配部分分别注明，预算尚未经过 GPU 训练实测。

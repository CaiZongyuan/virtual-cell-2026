...
**至此当前全部 9 门课（含附录 L2-06）的映射已齐**，本表可直接被 `scripts/check-lessons.py` 解析。一门课标为「—（豁免）」并写明理由：`00`（正文冻结 + 早于完成度契约；原 `L1-01` 的豁免随该课并入第 06 课而取消——第 06 课由 notebook 08/09 承接动手材料）。豁免是**登记在案的判断**，不是放宽判定——校验脚本里这条有带理由的 EXEMPT 记录。

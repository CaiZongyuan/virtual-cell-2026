...
| 06 | `08_foundation_model_routes.ipynb`、`09_simple_baselines.ipynb` | 五种赌注、基础模型三路线与效应迁移族的动手材料：08 做 scGPT 分层参数量与「为什么不给总数」、零表达靶点的接口可区分性、四分量 ANOVA 的 `beta_frac` 分档、B1 与「完美知道 Γ」上界的机会窗口（合成数据，纯 NumPy）；09 做 B0/B1 手算与 B2 相似度加权、LOCO 协议骨架与零覆盖回退 B0（合成数据，纯 NumPy）。**注**：notebook 09 由第 04 课与本课共用；B0–B3 的权威解释与手算口径在第 04 课 §6.3 |
| 07 | `11_ablation_and_falsification.ipynb`、`03_finetuning_counts_and_budget.ipynb` | 最终轮的判据与彩排：11 提供消融顺序表、证伪清单与「地板当阈值」的机械判定（稳定性优先于单次最高分，第 07 课 §3.3）；03 提供冻结清单里的输出合同与预算复算 |
| 附录 | `10_model_cards.ipynb` | 六张模型卡的可核对表（附录课）：可执行程度矩阵（代码/权重/真值/面板四列机械计数）、训练条件判定（本赛 D/E/F 能否满足「需要目标背景扰动真值」）、面板交集审计（示意集合，演示差集与 patch 边界问题）、靶点身份编码分类器（可外推 vs 查找表，含 B3 的 `b_t`）、分量归因能力判定（零依赖，纯标准库） |

`90_ref_*` / `91_ref_*` 是官方 Colab 的固定副本与中文对照，**不属于课程序列**，见下一节；`test.ipynb` 是环境自检，同样不映射任何课程。

**至此当前全部 9 门课（8 门主线 + 1 门附录）的映射已齐**，本表可直接被 `scripts/check-lessons.py` 解析。一门课标为「—（豁免）」并写明理由：`00`（正文冻结 + 早于完成度契约；原 `L1-01` 的豁免随该课并入第 06 课而取消——第 06 课由 notebook 08/09 承接动手材料）。豁免是**登记在案的判断**，不是放宽判定——校验脚本里这条有带理由的 EXEMPT 记录。

一门课可以映射多份 notebook（如第 04 课从噪声地板、证伪判据、B0–B3 手算到评分合同四段），也可以多门课共用一份（如第 03 课与第 07 课共用 `03`、第 04 课与第 06 课共用 `09`）。本表只声明「这门课的动手材料在哪里」，不要求一一对应。

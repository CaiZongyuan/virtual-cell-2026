...
> 本课回答：怎样复用 Arc 已发布的 State 预训练权重、完成微调并适配 2026 原始计数输出合同。**原理侧（State 为什么这样设计、损失为什么是 Energy 距离、哪些维度能迁移哪些必须重训）已移入 [L2-01 State 模型深潜](02-State模型拆解.md)；本课只保留微调所需的最小接口事实与命令，不重复讲解原理。**
>
> 前置课程：[第 03 课 单细胞原始计数生成](03-State上手.md)  
...
比赛采用 **10x Flex**。其基本信息链是：固定细胞中的 RNA 与基因对应的探针对结合，相邻探针连接，随后连接产物获得细胞条形码和分子标记，经过扩增和测序后归属到细胞与基因。**UMI（unique molecular identifier，唯一分子标识）**用于识别重复扩增的同一标记分子，避免把 PCR 复制数当成独立检测分子数。最终 raw counts 是这条测量流程下去重后的检测计数，不是细胞全部 RNA 分子的无损清点。分子过程详见[基础学习指南](00-VC2025学习指南.md)及[第 03 课 单细胞原始计数生成](03-State上手.md)。[S1][S12]
...
这一条决定了训练损失必须是集合级的：**换成逐行 MSE 会让模型塌缩到条件均值**。机制、闭式与实测对照见 [L2-01](02-State模型拆解.md) §4。

## 2. State 的两个模块，以及本教程微调哪一个（最小接口事实）

State 由两个模块组成：**SE（State Embedding，状态嵌入）**把表达向量变成紧凑表示，在 1.67 亿观察性细胞上预训练；**ST（State Transition，状态转移）**接收对照状态和扰动表示，预测扰动后的状态，在超过 1 亿个扰动细胞上训练。**只有 SE 权重不能得到一个扰动预测器**，本教程微调的对象是 ST。

选表达 / **HVG（highly variable genes，高变基因）**路线而不是 SE 路线，是为了省掉额外的编码器、解码器与基因轴核对；选出 2,000 个 HVG 不表示另外 16,533 个比赛基因不重要，也不满足全基因提交合同。[S3][S4]

> **为什么这样设计、每个模块解决什么问题、Energy 距离为什么不能换成 MSE、两套配置（328 与 768）为什么不能混用**：全部见 [L2-01 State 模型深潜](02-State模型拆解.md) §3、§4、§7。**本课不复述。**

本课只需要记住三条接口事实，其余维度在 §7.3 加载检查时逐项核对：

1. **一个细胞一个 token**，`cell_set_len=64` 指每组 64 个细胞不是 64 个基因。预测 400 个细胞不要求把训练集合长度改成 400——模型无位置编码，权重形状与集合长度无关，可以多组推理、处理尾组后精确保留 400 行。
2. **输入键是 `X_hvg`、`output_space=gene`**，基因轴 2,000（HVG）或 6,546（full），**都不是 18,533**。
3. **靶点默认是 one-hot，维数 2,024**，词表含义由 `pert_onehot_map.pt` 决定；本赛未见靶点需要换成连续表示（§3）。

## 3. 选择权重：不要把三种 State 当成同一个配置

以下为固定版本的官方资产或源码核验结果 [S3][S4][S5]。**「哪些维度能迁移、哪些必须重训」的完整判定表见 [L2-01](02-State模型拆解.md) §7。**

| 对象 | 已核实结构 | 用途 | 适配要点 |
|---|---|---|---|
| `ST-HVG-Replogle/zeroshot/jurkat` | 8 层；328；set 64；2,000；one-hot 2,024 | 最小加载与微调起点 | 保留原 HVG 轴；未见靶点表示；全基因输出；核实预训练暴露 |
...
| 当前源码 `model=state` 默认 | 8 层；768；set 512；全基因输出 | 随机初始化对照或后续扩模 | 不能直接用它接 328 维权重并声称完整复用 |
| 作者 VC2025 Colab | ESM2 连续靶点；40k 步；2025 H1 support | 参考靶点特征与训练入口 | 不是 2026 数据划分；不保证有对应 checkpoint |

**one-hot（独热编码）**把靶点表示为某一位置为 1、其余为 0 的向量，有利于已见靶点，不能自然处理新靶点。**ESM2** 从蛋白序列学习表示，能给新靶点一个向量——**能接收不等于知道它在新背景里的真实影响**；非蛋白编码靶点、别名与多个蛋白异构体还要单独处理。

特别注意 hidden 328 的官方配置仍有 `12 heads`、`head_dim=64`、`intermediate_size=3072`：投影后的**注意力内部宽度是 12×64=768，不等于 `hidden_dim=328`**，不能为了「能整除」自行改 head 数（[L2-01](02-State模型拆解.md) §2.3）。复制父模型的真实结构，比套用一般 Transformer 的经验规则可靠。

推荐两段推进：先用 `ST-HVG-Replogle` 验证加载、预处理和旧靶点行为；再复用其主干微调 ESM2 输入及全基因预测路径。后一段应明确命名为 **“State 预训练主干迁移微调”**，并报告复用了哪些权重。

## 4. 数据下载单：哪些必须有，哪些以后再加（实验或数据流程）
...
用 NTC 深度并非生物学定律：扰动可能改变细胞大小、总 RNA、存活率或测量质量。NTC 也已经含检测噪声，再直接作为 rate 做一次采样可能叠加噪声，因此需要平滑/收缩和生成器消融。独立逐基因 NB 能控制边缘方差，却不自动恢复基因间协方差。详见[第 03 课 单细胞原始计数生成](03-State上手.md)。

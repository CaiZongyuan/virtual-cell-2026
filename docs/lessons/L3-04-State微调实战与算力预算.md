...
> 本课回答：怎样复用 Arc 已发布的 State 预训练权重、完成微调并适配 2026 原始计数输出合同。**原理侧（State 为什么这样设计、损失为什么是 Energy 距离、哪些维度能迁移哪些必须重训）已移入 [L2-01 State 模型深潜](L2-01-State模型深潜.md)；本课只保留微调所需的最小接口事实与命令，不重复讲解原理。**
>
...
这一条决定了训练损失必须是集合级的：**换成逐行 MSE 会让模型塌缩到条件均值**。机制、闭式与实测对照见 [L2-01](L2-01-State模型深潜.md) §4。
...
> **为什么这样设计、每个模块解决什么问题、Energy 距离为什么不能换成 MSE、两套配置（328 与 768）为什么不能混用**：全部见 [L2-01 State 模型深潜](L2-01-State模型深潜.md) §3、§4、§7。**本课不复述。**
...
以下为固定版本的官方资产或源码核验结果 [S3][S4][S5]。**「哪些维度能迁移、哪些必须重训」的完整判定表见 [L2-01](L2-01-State模型深潜.md) §7。**
...
特别注意 hidden 328 的官方配置仍有 `12 heads`、`head_dim=64`、`intermediate_size=3072`：投影后的**注意力内部宽度是 12×64=768，不等于 `hidden_dim=328`**，不能为了「能整除」自行改 head 数（[L2-01](L2-01-State模型深潜.md) §2.3）。复制父模型的真实结构，比套用一般 Transformer 的经验规则可靠。

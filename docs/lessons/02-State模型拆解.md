...
>
> **改写进度**：第 03–07 课正在按新编号改写，本课对它们只做文字指路、暂不给链接。旧版 `L*-*` 系列仍在目录里，会被这一轮逐课吸收。
...
官方六项指标在第 04 课讲透。这里只需要一个够用的判据，来自 README §5 的三条成绩线：
...
**本课最关键的是第二条。** 你会看到 State 的损失函数是专门为了守住这一条而设计的——而最自然的那个替代品（MSE）会当场把它毁掉。完整的六项指标口径见第 04 课。
...
这四处是第 03 课的全部工作量，这里只列出来让你知道它们存在：
...
每一条要改哪一层、哪些权重能迁移、哪些必须重训，**完整版见第 03 课**。
...
这一节的话要说死，因为第 03 课整个计数生成模块的存在理由就在这里。
...
1. 官方要**原始整数计数**，State 输出的是 `log1p` 空间的浮点 → 第 03 课。
2. 官方的 DEG 指标会检查**单个基因**的方向与幅度，而能量距离作为集合损失不直接优化它 → 第 04 课。
...
怎么搭一个和本赛同难度的离线验证协议（leave-one-context-out），在第 03 课。

### 6.2 另一个必须记住的边界

论文自己报告：在遗传扰动数据（与本赛最接近的那一类）上，**某些基线在个别指标上偶尔优于 State**——线性模型在 DE 基因重叠上高出 20%，情境均值基线在倍数变化预测上高出 10% `[P1]`。

State 的优势是**跨指标的一致性**，不是单项第一。这一点直接决定了：**平凡基线在本赛里不是走过场**，它们是你判断「改进是不是真的」的参照系（第 04 课）。
...
`state tx train` / `state tx infer` 的完整命令、TOML 划分、环境安装在第 03 课——**runbook 只有一个权威位置，本课不复述。**
...
3. **它必须区分三种「零」**：未测、测得零、生物学关闭（01 §4.3）。State 在 `log1p` 空间训练、末端走 `ReLU`，这条链路上「零」的语义是被压过的，本赛的稀疏计数合同要求你在输出端重新处理它（第 03 课）。

> **够用版：什么是噪声地板。** 同一份真实数据重复抽样，分数本身就会抖动；这个抖动的幅度就是地板。**改动前后分数差不超过地板，就不算改进。** 它怎么测、本机实测是多少，在第 04 课。
...
第 1、4、7 项是真正的工作量。**落地命令和步骤在第 03 课，本课只负责说明「为什么必须改这几处」。**
...
- 训练超参（`lr=1e-4`、Adam 而非 AdamW、`clip=10`、无内置 LR schedule、默认未启用 bf16）→ 第 03 课。
- 数据划分、对照匹配、缺测基因 mask、CP10K 与 log1p 尺度 → 第 03 课的数据处理合同。
- 六项官方指标的定义与离线实现 → 第 04 课。
- 噪声地板的测量方法与本机实测值 → 第 04 课。

...
> 论文结论一律 `[P1]`，**未在本项目复现**。
>
> **改写进度**：第 04–07 课正在按新编号改写，本课对它们只做文字指路、暂不给链接。旧版 `L*-*` 系列仍在目录里，会被这一轮逐课吸收。
...
> ⚠️ **一个反直觉的前提：微调集里没有某背景，不代表模型没见过它。** 假设父权重已经在 Jurkat 上训练过，之后你只用 K562 微调、再在 Jurkat 上测试——这**不能**叫「Jurkat 从未见过」。预训练权重已经携带了那个背景的信息。所以每个实验要记**两个**清单：父 checkpoint 的训练暴露、本次微调的数据暴露。完整判据在第 04 课。
...
**正确图景**：采样器不能修复错的 rate。§8.2 的六步里，第 ② 步（预测全基因 rate）是**上游**，第 ⑤ 步（生成整数）是**下游**；下游永远只能包装上游。判断「改进有没有用」需要先有一个噪声地板（第 04 课）。
...
- **六项完整指标的定义、口径与离线实现** → 第 04 课。
- **噪声地板的测量方法与本机实测值** → 第 04 课。本课 §11.1 的题一故意留了这个缺口：判断「改进是否为真」需要它。
- **验证划分的完整方法**（LOTO、双重留出、七处泄漏点清单） → 第 04 课。本课只给 LOCO 的定义与「预训练暴露也要算」这一条纪律。
- **B0–B3 平凡基线与效应迁移族** → 第 04 课（B0–B3 的完整推导留在备选架构附录并回指）。
- **Stack 的架构、上下文机制与 ICL 取舍** → 第 05 课。
- **基础模型三条路线与五种赌注** → 第 06 课。
- **最终轮的冻结清单、集成策略与提交 runbook** → 第 07 课。

# 05｜Stack：推理时把无标签细胞当示例

> **本课回答**：State 之外的另一条路——Stack 为什么不需要为每个任务重训：它怎样把「示例细胞」变成输入的一部分，这个设计的代价是什么，以及它在本赛合同下哪里会直接失效。读完你能做一件事：判断 in-context 与微调在本赛合同下该选哪条，以及选了 ICL 之后上下文里该放什么。
>
> **前置**：[[第 02 课](02-State模型拆解.md) State 模型拆解](02-State模型拆解.md)（本课处处与它对照）、[[第 04 课](04-你怎么知道改进是真的.md) 你怎么知道改进是真的](04-你怎么知道改进是真的.md)（§7 的自举判据直接用它的噪声地板）。[第 03 课](03-State上手.md) 的 LOCO 协议与计数生成器在 §7 回指。
>
> **下一课**：[[第 06 课](06-其余路线全景.md) 其余路线全景](06-其余路线全景.md)。
>
> 核验日期：2026-09-18。面向熟悉软件开发和机器学习、正在补齐单细胞生物学的读者。
>
> **本课的证据性质**：本文是**源码与论文对照阅读**的结果。全部结构、维度、窗口切分与计划表取自固定 commit 的官方仓库与已发布配置（**静态核验**）；论文结论一律标 `[P1]`，**未在本项目复现**。本机**未安装 Stack、未下载权重、未运行推理**，因此显存与算力数字**全部是手算**，并显式标为工程假设；**耗时一个数字都没有**。窗口切分的整数结果是源码逻辑复刻，由 [notebook 07](../../notebook/07_stack_icl_context.ipynb)（纯 NumPy，`SEED=20260917`）执行验证。
>
...
04 结束时，你手里有**一台判定器**：任何「改进」都要过它——噪声地板、六项指标的完整口径、消融顺序表、证伪清单，外加四条最便宜的对照基线 B0–B3。
...
**State 微调不是唯一的路线。** 官方 FAQ 允许的另一个模型 Stack，走了与 State 完全相反的一步：它不把背景和扰动编码成条件，而是把「例子」直接摆到模型面前——**上下文学习（in-context learning, ICL）**：推理时把一批细胞和它们的标签一起放进输入，模型通过细胞间注意力从这批例子里取信息。于是同一个权重可以不做任何调整就切到新任务、新细胞系、甚至新物种。

```text
两条路线的入口

  State： (背景, 靶点) ────编码成条件────→ 模型 ──→ 生成分布    第 02–03 课
  Stack： (示例细胞 + 标签) ─摆进上下文─→ 模型 ──→ 生成分布    本课
                                       ▲
                        上下文里必须有「已扰动细胞」
                        本赛给新背景的只有 NTC —— §7 是本课真正的输出
```

**一句话概括：Stack 把「学一个扰动任务」换成了「看几个例子，然后照做」。** 这个决定带来两个后果：它可能绕开「新背景只有非靶向对照（non-targeting control, NTC）」的死结，也可能因为这个死结而完全用不上——§7 就是讲清它到底落在哪一边。

> **一个贯穿全课的例子。** 机制部分继续用**背景 A、靶点 `ADNP`**（`ADNP` 是 01 §6.2 的示意占位符，本课不对它做任何生物学断言）；讲到 §5 与 §7 时会换到**背景 D**（新背景，只给 NTC）——因为 Stack 与本赛合同的冲突恰恰发生在新背景上。

![Stack 的双轴注意力、代价与窗口切分](assets/vc2026-course/L2-02-stack-icl.webp)

*图 1：(a) 前向路径与两个注意力轴；(b) 两个轴的显存代价对比与 8 GiB 边界；(c) 窗口切分公式与查询细胞被复制的后果。所有数字来自固定 commit `cacc2e4b` 的源码静态核验与 notebook 07 的手算，**不是实例化实测**；图上每个数字的来源与证据等级见 `docs/style/prompts/L2-02-stack-icl-facts.md`（文件名保留生成时的旧课号）。*

---

## 1. 它赌什么

**Stack 押的是课程地图里「表征预训练」这一注**（五种赌注的权威归属见[课程索引](README.md) §8），但把这一注押得比同族（scGPT、scBaseCount、AIDO Cell）更极端。核心假设是一句话：
...
**与[第 02 课](02-State模型拆解.md) State 的关键分野：** State 赌「条件化生成」——把背景与扰动编码成条件，直接生成目标分布；Stack 赌「上下文推断」——把示例直接摆到模型面前。前者学的是 `p(细胞 | 背景, 扰动)`，后者学的是「给我看几个 `(细胞, 标签)` 对，我推断这个映射」。
...
3. **它仍然属于「表征预训练」赌注，因此继承这一族的天花板判据。** 课程索引 §8 归属的天花板判据列出了三条独立基准工作（Ahlmann-Eltze 等、Csendes 等、PerturBench），都报告：**在本任务上，大规模预训练表征的收益经常是负的，没有一个深度模型稳定优于相应的简单基线**（完整口径在[第 04 课](04-你怎么知道改进是真的.md) §1.1）。Stack 的上下文学习是一次绕过这个判据的尝试（因为它不用微调、不学固定任务头），但**它并没有被那三个基准检验过**——那三篇比的是 scGPT、scFoundation 一类表征模型，不是 ICL 模型。这是本课必须写明的证据缺口，不能当作「Stack 已经证明了它不适用」。

---

## 2. 输入输出契约：一个细胞是一段 token 序列
...
**这是与 State 完全相反的组织方式。** State 里**一个细胞是一个 token**（[第 02 课](02-State模型拆解.md) §2.1）；Stack 里**一个细胞是一段长度为 `n` 的 token 序列**，细胞才是外层。所以 State 的注意力在「细胞之间」，Stack 的注意力**在两个轴上都有**——这就是下一节的主角。

### 2.2 两套维度，别合并成一句话

Stack 有预训练（Large）与后训练（Large-Aligned）两档配置，逐项对照沉到 §9.3 的速查表。这里只点三处最容易合并出错的：

1. **预训练与后训练用的是两套 `K`**（256 vs 512）——§2.3 会证明这只是推理窗口大小，不是结构约束；
2. **`G = 15,012` 与本赛 `18,533` 的差额**是必须处理的基因轴适配（§7 第 2 项）；
3. **Stack 输出 NB 参数，本赛要非负整数 counts**（§7 第 3 项）——不过这比 State 少一步（§7 第 3 项会说为什么）。
...
这与[第 02 课](02-State模型拆解.md) §2.4 第 2 项（State 的 `cell_set_len` 与权重形状无关）**完全同构**。两课合起来给出一条贯穿本项目的操作规则：
...
预训练档（`batch=32, K=256`）比这还大。**结论：Stack 在本机属于 [课程索引](README.md) §6 的「纯阅读」档，本机不做训练也不做真实推理。**

**耗时：本课不给任何数字。** 一个 `K=512`、batch=1 的窗口前向约 **2.4×10¹¹ FLOPs** 量级（主导项是细胞间注意力的 `2K²d` 与 `output_mlp` 的 `4ndG`）——但把它换算成秒需要「GPU 有效算力 × 达成率」两个数，本机没跑过，不代填。这是本课与[第 02 课](02-State模型拆解.md) §10.3 一致的口径：**能手算的给手算，不能实测的不编造。**
...
模型输出的不是表达值，而是**负二项分布（negative binomial, NB）的两个参数**（[S1] `output_mlp: n·d → 2·n·d → 2G`）：
...
**为什么是负二项而不是 MSE？** 因为 scRNA-seq 的原始计数是**过度离散**的：方差大于均值。泊松分布强制 `方差 = 均值`，会把真实数据的离散度抹掉；负二项多一个 `theta` 参数，可以独立表达「这个基因在该细胞里的波动有多大」。**这一条直接关系到本赛的群体分布线**（[课程索引](README.md) §5.2 的方差、零比例、基因间协同）——一个只会输出均值的模型在这条线上必然失分。这一点与 State 用 energy 距离保留集合离散度是**同一个问题的两种解法**：State 在损失层面保方差，Stack 在输出分布层面保方差。
...
后训练阶段（`Stack-Large-Aligned`）用**冻结的预训练模型当教师**，做自蒸馏（self-distillation）（[S1] `models/finetune/model.py`、`mixins.py`）：
...
| 项 | 含义 | 作用 |
|---|---|---|
| `L_dist` | `0.5 × (L_embed + L_gene)`，用 **energy 距离** | 让预测分布与真实分布一致——**与[第 02 课](02-State模型拆解.md) §4.1 是同一个数学对象** |
...
![T=5 的迭代生成计划](assets/vc2026-course/05-icl-generation-plan.webp)

*图 2：T=5 的迭代生成计划。左：两张表（`mr` 递减、`cr` 递增）与每步的提示/查询窗口切分（K=512，五根条的比例即提示/查询占比）；右：五步明细，第 5 步 `mask_rate=0` 是对完整窗口的一致性前向。数字全部来自源码 `get_incontext_generation` 静态核验，非实测。*
...
这不是「模型没学过」，而是「**上下文根本没有东西可放**」。它是数据可得性问题，不是模型能力问题。落在本项目的处理方式见[第 03 课](03-State上手.md)的数据 manifest 与[[第 04 课](04-你怎么知道改进是真的.md) §6.3](04-你怎么知道改进是真的.md)的 B0–B3 基线对照。

**使用条款：** 与[第 02 课](02-State模型拆解.md) §5.2 同一条——非商业参赛者可使用预训练 checkpoint，商业参赛者使用预训练权重需相应商业许可 [S2]。这不是法律意见。
...
![窗口切分与 137 个重复行](assets/vc2026-course/05-window-split-duplicates.webp)

*图 3：窗口切分与 137 个重复行。上左：源码切分公式（与论文正文的「25%」说法不一致）；上右：本赛 400 个细胞的三档切分，`cr=0.40` 时 3 个窗口、137 个查询细胞被复制；下：复制行的来源（`test_indices[:137]`，复制开头的查询细胞 0–136）。切分为源码逻辑复刻，整数结果经 notebook 07 单元一（纯 NumPy，`SEED=20260917`）执行验证。*
...
Stack 的 `stack generate` CLI、`--prompt-ratio` 等参数的完整用法、以及权重下载与许可核对，**本课不复述**——按 [课程索引](README.md) §7「不重复研究文档」的约定，本课只保留机制与接口层，命令落在 `references/stack/README.md` 与后续 runbook。
...
所以本课属于 [课程索引](README.md) §6 的「**纯阅读**」档：不要求产物。本机真正能做的是三件事——读源码与配置、跑 [notebook 07](../../notebook/07_stack_icl_context.ipynb) 的纯 NumPy 复刻、用手算把显存账与参数量算出来。**其中参数量手算与论文表 3 对上了，这是本课唯一一次与官方数字的直接碰撞。**
...
上下文里放什么？——三条绕法的决策点
  ...
  新背景 D，靶点 X，只有 NTC
        │
        ├── A. 跨背景示例：放背景 A/B/C 或公开筛选里
        │      同靶点的已扰动细胞
        │      └─ 赌「扰动效应跨背景可迁移」← 本领域最脆弱的假设
        │         检验协议 = 第 03 课的 leave-one-context-out
        │
        ├── B. 自举：先 NTC-only 生成一轮，把预测细胞当示例
        │      └─ 赌「第一轮预测已超过噪声地板」
        │         判据 = 第 04 课的地板：不超阈值，B 从起点就不成立
        │
        └── C. 只放 NTC：上下文不带任务信息
               └─ 不赌任何迁移假设，但 ICL 退化成条件编码器
                  等价于回到第 03 课的 State 微调主线
```

| 绕法 | 具体做法 | 代价 / 前提 |
|---|---|---|
| **A. 跨背景示例** | 从 A/B/C 或公开 Perturb-seq 里取同靶点的扰动细胞，放进上下文 | 前提是「扰动效应跨背景可迁移」——这是本领域最脆弱的假设之一（课程索引 §8 的五种赌注归属），且[第 03 课](03-State上手.md)的 leave-one-context-out 协议就是用来检验它的 |
...
**这三条对应三种完全不同的实施成本。** 本项目首投选择的是 C→微调路线（[第 03 课](03-State上手.md)的 State 主线），把 A/B 留作对照——A 的前提检验协议是[第 03 课](03-State上手.md)的 LOCO，B 的前提判据是[第 04 课](04-你怎么知道改进是真的.md)的噪声地板；效应迁移族的完整决策规则由[第 06 课](06-其余路线全景.md)给出。**注意这不是说 Stack 无用**：Stack 的 NB 输出头（§7 第 3 项）是它对本赛最直接的贡献点——它天生输出计数分布，比 State 少一步适配。
...
跑完 notebook 后请填 §9.2 的表——那是本课要求你留下的产物。
...
|---|---|
| 它赌什么 | §1 三个赌 + §1.1 三条必输 |
| 输入输出契约 | §2 张量形状、两套维度、`K` 不是结构约束 |
| 架构 | §3 双轴注意力与 8 GiB 显存账 |
| 训练目标 | §4 NB 似然、自蒸馏与 `T=5` 推理 |
| 数据 | §5 语料规模与「零样本」口径差 |
| 官方代码怎么跑 | §6 入口、两条核验命令与本机边界 |
| 适配缺口 | §7 五项缺口与三条绕法 |
| 诊断题 | §10 |
...
左两列都是官方事实（[S1] 两份 YAML 配置），但**不是同一个运行点**——预训练与后训练的两套 `K` 只是推理窗口（§2.3），别当成两种结构。
...
1. **A 赌「扰动效应跨背景可迁移」**——即 `X` 在背景 A 里的效应能代表它在背景 D 里的效应。这是本领域最脆弱的假设之一（课程索引 §8 的五种赌注归属），检验协议是[第 03 课](03-State上手.md)的 leave-one-context-out。**B 赌「模型的第一轮预测已经比 NTC 更接近真值」**——即自举的起点是有效的（如果不成立，就是把噪声当示例）。**C 赌「上下文不需要携带任务信息」**——也就是放弃 ICL。
...
1. **137 行是重复的**（[notebook 07](../../notebook/07_stack_icl_context.ipynb) 单元一的实测输出：`cr=0.40` 档）。依据是源码 `get_incontext_prediction` 的尾组处理：`n_test = int(512 × (1 − 0.65)) = 179`，400 个查询细胞需要 3 个窗口（`179 × 3 = 537 > 400`），最后一个窗口只有 `400 − 358 = 42` 个真实查询细胞，缺 `179 − 42 = 137` 个，源码用 `test_indices[:137]` 补齐——**这 137 个细胞是开头那批的副本**。
...
3. **主要伤害群体分布线**（[课程索引](README.md) §5.2）：重复行对方差的贡献是 0（同一行跟自己的差是 0），对零比例的贡献是把该细胞的特征重复计数。总 UMI 的均值不受影响（复制不改变均值），但**方差、零比例、细胞状态组成、基因间协变全部被拉偏**。DEG 线也会受轻度影响（伪批量层面被重复行的权重放大）。**工程合同线不受影响**——这正是它危险的地方：**合同线全绿，分布线已经错了。**
...
4. **§1.1 第 3 条的证据缺口已显式写明**：课程索引 §8 归属的「表征预训练」判据所列三篇基准（Ahlmann-Eltze 等、Csendes 等、PerturBench）**没有检验过 ICL 类模型**，因此「基础模型打不过线性基线」这一判据**不能直接外推到 Stack**。这是缺口，不是结论。
5. **未引用任何参赛者自报数字。** Stack 在 VC2025 上的实际表现属于参赛者自报（见 `docs/blogs/`），本课一概不用。

### 11.4 主题边界（本课不承担的内容）

- **Stack 的架构、训练目标与上下文机制是本课的权威内容。** State 的对应内容是[第 02 课](02-State模型拆解.md)的权威内容，本课只在机制对照处回指。
- **噪声地板**的权威位置是[第 04 课](04-你怎么知道改进是真的.md)，本课 §1.1、§7.1、§10.1 只引用它的结论。
...
- **五种赌注与天花板判据**的权威位置是[第 06 课](06-其余路线全景.md) §1.3，本课 §1 只回指「表征预训练」这一注，不重复展开其他四注。
- **in-context 与微调的取舍判断**：完整版是本课 §7.1 加[第 03 课](03-State上手.md)的微调路线与算力预算；「ICL 增益是否为真」的判据完整版在[第 04 课](04-你怎么知道改进是真的.md) §3，本课只用其结论。
- **基础模型三条路线（scGPT / scBaseCount / AIDO Cell）** 的横向对照由[第 06 课](06-其余路线全景.md)给出，本课不展开。

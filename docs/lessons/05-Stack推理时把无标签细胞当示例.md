...
>
> **前置**：[第 02 课 State 模型拆解](02-State模型拆解.md)（本课处处与它对照）、[第 04 课 你怎么知道改进是真的](04-你怎么知道改进是真的.md)（§7 的自举判据直接用它的噪声地板）。[第 03 课](03-State上手.md) 的 LOCO 协议与计数生成器在 §7 回指。
>
> **下一课**：06《其余路线全景：效应迁移 / 基础模型 / 分解式 / 世界模型》。
>
...
>
> **改写进度**：本课由旧课 `L2-02` 全文合并重写，旧课将在验收后删除。第 06、07 课尚未改写，本课对它们只作文字指路、暂不给链接。
...
**与第 02 课 State 的关键分野：** State 赌「条件化生成」——把背景与扰动编码成条件，直接生成目标分布；Stack 赌「上下文推断」——把示例直接摆到模型面前。前者学的是 `p(细胞 | 背景, 扰动)`，后者学的是「给我看几个 `(细胞, 标签)` 对，我推断这个映射」。
...
3. **它仍然属于「表征预训练」赌注，因此继承这一族的天花板判据。** 课程索引 §8 归属的天花板判据列出了三条独立基准工作（Ahlmann-Eltze 等、Csendes 等、PerturBench），都报告：**在本任务上，大规模预训练表征的收益经常是负的，没有一个深度模型稳定优于相应的简单基线**（完整口径在第 04 课 §1.1）。Stack 的上下文学习是一次绕过这个判据的尝试（因为它不用微调、不学固定任务头），但**它并没有被那三个基准检验过**——那三篇比的是 scGPT、scFoundation 一类表征模型，不是 ICL 模型。这是本课必须写明的证据缺口，不能当作「Stack 已经证明了它不适用」。
...
**这是与 State 完全相反的组织方式。** State 里**一个细胞是一个 token**（第 02 课 §2.1）；Stack 里**一个细胞是一段长度为 `n` 的 token 序列**，细胞才是外层。所以 State 的注意力在「细胞之间」，Stack 的注意力**在两个轴上都有**——这就是下一节的主角。
...
这与第 02 课 §2.4 第 2 项（State 的 `cell_set_len` 与权重形状无关）**完全同构**。两课合起来给出一条贯穿本项目的操作规则：
...
**耗时：本课不给任何数字。** 一个 `K=512`、batch=1 的窗口前向约 **2.4×10¹¹ FLOPs** 量级（主导项是细胞间注意力的 `2K²d` 与 `output_mlp` 的 `4ndG`）——但把它换算成秒需要「GPU 有效算力 × 达成率」两个数，本机没跑过，不代填。这是本课与第 02 课 §10.3 一致的口径：**能手算的给手算，不能实测的不编造。**
...
| 项 | 含义 | 作用 |
|---|---|---|
| `L_dist` | `0.5 × (L_embed + L_gene)`，用 **energy 距离** | 让预测分布与真实分布一致——**与第 02 课 §4.1 是同一个数学对象** |
...
这不是「模型没学过」，而是「**上下文根本没有东西可放**」。它是数据可得性问题，不是模型能力问题。落在本项目的处理方式见[第 03 课](03-State上手.md)的数据 manifest 与[第 04 课 §6.3](04-你怎么知道改进是真的.md)的 B0–B3 基线对照。

**使用条款：** 与第 02 课 §5.2 同一条——非商业参赛者可使用预训练 checkpoint，商业参赛者使用预训练权重需相应商业许可 [S2]。这不是法律意见。
...
**这三条对应三种完全不同的实施成本。** 本项目首投选择的是 C→微调路线（第 03 课的 State 主线），把 A/B 留作对照——A 的前提检验协议是第 03 课的 LOCO，B 的前提判据是第 04 课的噪声地板；效应迁移族的完整决策规则由第 06 课给出。**注意这不是说 Stack 无用**：Stack 的 NB 输出头（§7 第 3 项）是它对本赛最直接的贡献点——它天生输出计数分布，比 State 少一步适配。
...
- **五种赌注与天花板判据**的权威归属见[课程索引](README.md) §8（当前在 `L1-01`，第 06 课吸收后随迁），本课 §1 只回指「表征预训练」这一注，不重复展开其他四注。
- **in-context 与微调的取舍判断**：完整版是本课 §7.1 加[第 03 课](03-State上手.md)的微调路线与算力预算；「ICL 增益是否为真」的判据完整版在[第 04 课](04-你怎么知道改进是真的.md) §3，本课只用其结论。
- **基础模型三条路线（scGPT / scBaseCount / AIDO Cell）** 的横向对照由第 06 课给出，本课不展开。

...
> 前置课程：[L3-01 评分指标与离线评估](L3-01-评分指标与离线评估.md)  
> 下一课：[L3-03 单细胞原始计数生成](L3-03-单细胞原始计数生成.md)  
...
> **配套 notebook：[`01_vc2026_data.ipynb`](../../notebook/01_vc2026_data.ipynb) 与 [`03_finetuning_counts_and_budget.ipynb`](../../notebook/03_finetuning_counts_and_budget.ipynb)。** 本节第 1–4 步（manifest、结构审计、基因对齐、split）落在前者：它扫描 A/B/C 三个背景的全部 55,200 个细胞，演示 AnnData 的三个坐标系、NTC guide 分组与 CSR 存储——这正是「把每份数据的扰动方式、背景、面板、化学与批次写进合同」能被执行的地方。第 5 步的划分泄漏侧落在后者：`03` 有一张「预训练暴露也计入划分」的检查清单，说明父 checkpoint 见过某背景后，只在微调 TOML 里删掉它**不能**称为干净 LOCO。
>
> **边界写清**：本课讲的公共数据集（VC2025 H1、Replogle K562/RPE1、Nadig HepG2/Jurkat、Jiang 六细胞系）**本机尚未下载**——四组候选的 manifest 只填了官方候选页写明的字段，其余标为待核验。因此上面第 3 步「通过闸门后下载子集」与第 5 步「用六项指标运行三个朴素基线」**都是待执行的计划，不是已完成的记录**。两份 notebook 覆盖的是合同、审计与泄漏检查的**方法**，不是这四组数据的实际运行结果。

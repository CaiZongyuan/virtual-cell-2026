...
同日后续[检查点微调核验](state-checkpoint-finetuning-audit.md)与[第 03 课：State 上手](../lessons/03-State上手.md)补充了三项执行细节：原生 `init_from` 可微调；`ckpt_every_n_steps` 在当前 callback 中未使用，实际由 `val_freq` 控制保存；参数总量还可能包含辅助 `gene_decoder`，下文主 forward 路径的手算不等于完整模型参数数目。梯度累积后，验证间隔按训练 batch、checkpoint 间隔按 optimizer steps，正式选模应修正并核验保存逻辑。

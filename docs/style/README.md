...
| `sources/07-final-figures.html` | [第 07 课](../lessons/07-最终轮冻结集成与提交.md)「最终轮作战板」HTML/CSS 精确图源；1 幅（D/E/F 到达后的五步执行链与三条红线） | 本仓库生成；2026-09-18；无外部运行时资源 | 用 `?figure=timeline` 选择画面，以 Chromium 1600x840 @ `deviceScaleFactor=2` 截图；导出走 `scripts/html-cdp-shot.py`，LANCZOS 降到 2200 px → WebP q86，成品 `docs/lessons/assets/vc2026-course/07-final-timeline.webp`（2200×1155，204.6 KB） |
...
| `sources/09-figures.html` | 原第 9 课（现 [第 07 课](../lessons/07-最终轮冻结集成与提交.md)）HTML/CSS 精确图源。**文件名保留旧课号**（原第 9 课，溯源约定同上） | 本仓库生成；无外部运行时资源 | 用 `?figure=final` 渲染最终轮事件驱动流水线 |
